...
function computePriceImpact<TBase extends Currency, TQuote extends Currency>(
  ...
  outputAmount: CurrencyAmount<TQuote>): Percent {
 ...
 */
function wrappedAmount(currencyAmount: CurrencyAmount<Currency>, chainId: ChainId): TokenAmount {
  if (currencyAmount instanceof TokenAmount) return currencyAmount
  if (currencyAmount.currency instanceof Ether) return new TokenAmount(WETH[chainId], currencyAmount.raw)
  invariant(false, 'CURRENCY')
}

function wrappedCurrency(currency: Currency, chainId: ChainId): Token {
  if (currency instanceof Token) return currency
  if (currency instanceof Ether) return WETH[chainId]
  invariant(false, 'CURRENCY')
}
...
export class Trade {
   ...
   */
  public readonly nextMidPrice: Price<Currency, Currency>
  /**
   ...
   */
  public static exactOut(route: Route, amountOut: CurrencyAmount<Currency>): Trade {
    return new Trade(route, amountOut, TradeType.EXACT_OUTPUT)
  }

  public constructor(route: Route, amount: CurrencyAmount<Currency>, tradeType: TradeType) {
    ...
    this.inputAmount =
      tradeType === TradeType.EXACT_INPUT
        ? amount
        : route.input instanceof Ether
        ? CurrencyAmount.ether(amounts[0].raw)
        : amounts[0]
    this.outputAmount =
      tradeType === TradeType.EXACT_OUTPUT
        ? amount
        : route.output instanceof Ether
  ...
  public static bestTradeExactOut(
    ...
    // used in recursion.
    currentPairs: Pair[] = [],
    originalAmountOut: CurrencyAmount<Currency> = currencyAmountOut,
    bestTrades: Trade[] = []
  ): Trade[] {
    invariant(pairs.length > 0, 'PAIRS')
    invariant(maxHops > 0, 'MAX_HOPS')
